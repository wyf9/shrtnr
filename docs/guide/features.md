...
- **动态重定向规则**：支持 `_redirects` 风格的占位符（`:name`）与通配符（`*` → `:splat`），便于迁移旧的重定向路径。详见 [动态重定向规则](/guide/redirect-rules)。
...
- **SDK**：TypeScript ([`@oddbit/shrtnr`](https://oddb.it/shrtnr-npm-readme))、Python ([`shrtnr`](https://oddb.it/shrtnr-pypi-readme))、Dart/Flutter ([`shrtnr`](https://oddb.it/shrtnr-pub-readme))。详见 [SDK](/integrations/sdks)。
- **内置 MCP 服务器**：位于 `/_/mcp`，通过 Cloudflare Access OAuth 授权，让 Claude、Copilot 等 AI 助手创建和管理短链。详见 [MCP 服务器](/integrations/mcp)。
- **浏览器扩展**：Chrome / Firefox 扩展，一键短链当前标签页。详见 [浏览器扩展](/integrations/browser-extensions)。
