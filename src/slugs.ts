...
export function generateRandomSlug(length: number): string {
  ...
  return Array.from(
    bytes,
    (b) => RANDOM_CHARSET[b % RANDOM_CHARSET.length],
  ).join("");
}
...
export function validateRandomSlug(slug: string): string | null {
  ...
  if (!RANDOM_SLUG_REGEX.test(slug))
    return "Slug must contain only alphanumeric characters";
  return null;
}
...
export function validateCustomSlug(slug: string): string | null {
  ...
  if (!CUSTOM_SLUG_REGEX.test(slug))
    return "Custom slug must start and end with a letter or number; allowed symbols in the middle: . _ ~ -";
  return null;
}

export function validateSlugLength(length: number): string | null {
  if (!Number.isInteger(length) || length < MIN_SLUG_LENGTH)
    return `Slug length must be an integer >= ${MIN_SLUG_LENGTH}`;
