...
export class LinkRepository {
  static async list(
    ...
    const slugs = await db
      .prepare(
        `SELECT ${slugSel.sql} FROM slugs s ORDER BY is_custom ASC, created_at ASC`,
      )
      .bind(...slugSel.binds)
      .all<Slug>();

    return (links.results ?? []).map((link) => {
      const linkSlugs = (slugs.results ?? []).filter(
        (s) => s.link_id === link.id,
      );
  ...
  static async getById(
    ...
    const slugs = await db
      .prepare(
        `SELECT ${slugSel.sql} FROM slugs s WHERE link_id = ? ORDER BY is_custom ASC, created_at ASC`,
      )
  ...
  static async findByUrl(
    ...
    const results = await Promise.all(
      ids.map(({ id }) => LinkRepository.getById(db, id, opts)),
    );
  ...
  static async create(
    ...
    const [linkResult] = await db.batch([
      db
        .prepare(
          "INSERT INTO links (url, label, created_at, expires_at, created_via, created_by) VALUES (?, ?, ?, ?, ?, ?)",
        )
        .bind(
          data.url,
          data.label ?? null,
      ...
      db
        .prepare(
          "INSERT INTO slugs (link_id, slug, is_custom, is_primary, created_at) VALUES (last_insert_rowid(), ?, ?, 1, ?)",
        )
  ...
  static async update(
    ...
    const expiresAt =
      updates.expires_at !== undefined ? updates.expires_at : link.expires_at;

    await db
      .prepare(
        "UPDATE links SET url = ?, label = ?, expires_at = ? WHERE id = ?",
      )
  ...
  static async disable(
    ...
    const now = Math.floor(Date.now() / 1000);
    await db
      .prepare("UPDATE links SET expires_at = ? WHERE id = ?")
      .bind(now, id)
      .run();
    return LinkRepository.getById(db, id);
  }
  ...
  static async enable(
    ...
    if (!link) return null;
    await db
      .prepare("UPDATE links SET expires_at = NULL WHERE id = ?")
      .bind(id)
      .run();
    return LinkRepository.getById(db, id);
  }
  ...
  static async delete(db: D1Database, id: number): Promise<boolean> {
    ...
    await db
      .prepare(
        "DELETE FROM clicks WHERE slug IN (SELECT slug FROM slugs WHERE link_id = ?)",
      )
      .bind(id)
      .run();
  ...
  static async findByOwner(
    ...
    const rows = await db
      .prepare(
        "SELECT id FROM links WHERE created_by = ? ORDER BY created_at DESC",
      )
      .bind(owner)
      .all<{ id: number }>();
