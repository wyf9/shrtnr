...
describe("computeVisitorFingerprint", () => {
  ...
  it("is deterministic for the same visitor within a day", async () => {
    const fp1 = await computeVisitorFingerprint(
      "1.2.3.4",
      "Mozilla/5.0",
      "secret",
      now,
    );
    const fp2 = await computeVisitorFingerprint(
      "1.2.3.4",
      "Mozilla/5.0",
      "secret",
      now,
  ...
  it("differs between different IPs", async () => {
    const a = await computeVisitorFingerprint(
      "1.2.3.4",
      "Mozilla/5.0",
      "secret",
      now,
    );
    const b = await computeVisitorFingerprint(
      "5.6.7.8",
      "Mozilla/5.0",
      "secret",
      now,
  ...
  it("rotates across days for the same visitor", async () => {
    const day1 = await computeVisitorFingerprint(
      "1.2.3.4",
      "Mozilla/5.0",
      "secret",
      new Date("2026-04-22T10:00:00Z"),
    );
    const day2 = await computeVisitorFingerprint(
      "1.2.3.4",
      "Mozilla/5.0",
      "secret",
  ...
  it("produces a fixed-length hex string", async () => {
    const fp = await computeVisitorFingerprint(
      "1.2.3.4",
      "Mozilla/5.0",
      "secret",
      now,
