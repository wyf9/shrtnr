...
describe("SettingRepository.get", () => {
  it("returns the value for an existing setting", async () => {
    const val = await SettingRepository.get(
      env.DB,
      "anonymous",
      "slug_default_length",
    );
...
describe("SettingRepository.set", () => {
  ...
  it("scopes settings by identity", async () => {
    ...
    expect(
      await SettingRepository.get(env.DB, "user-a@example.com", "theme"),
    ).toBe("dark");
    expect(
      await SettingRepository.get(env.DB, "user-b@example.com", "theme"),
    ).toBe("light");
  });
});
