...
- Links can now be deleted regardless of click count. `deleteLink` and `LinkRepository.delete` dropped the guard that blocked removing any link with recorded clicks, so `DELETE /_/api/links/:id`, the admin UI, and the MCP `delete_link` tool now purge the link, its slugs, and its entire click history in one step. Only the link owner can delete, and deletion is permanent, so disable a link instead when you want to keep its analytics. The link-detail three-dot menu always offers **Delete** now (alongside **Disable**/**Enable**), and its confirmation dialog warns how many clicks will be discarded when the link has any.
- The target URL of an existing link is now editable from the admin dashboard. The link-detail page shows an inline editor next to the destination, wired to the existing `PUT /_/api/links/:id` update path, so you can repoint a short link without recreating it. New copy (`linkDetail.editUrl`, `linkDetail.urlPlaceholder`, `linkDetail.confirmDeleteWithClicks`, `client.urlUpdated`, `client.urlError`) is translated across English, Bahasa Indonesia, Svenska, and Simplified Chinese.
